...
g19.3 <- datos.grafico %>% 
  ...
  ggtitle('Días de Cuarentena Acumulada, según Zona')
...
g19.4 <- datos.grafico %>% 
  ...
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  ggtitle('Días de Cuarentena Acumulada, según Tramo Etario')
...
g19.5 <- datos.grafico %>% 
  ...
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  ggtitle('Cumplimiento de Aislamiento Social, según Días de Cuarentena Acumulada')

g19.5
```

```{r ccum-depr}
datos.grafico <- data.frame((svytable(~depr + ccum_t, 
                                      elsoc_diseno_3, round = F))) %>% 
  group_by(ccum_t) %>% 
  mutate(porcentaje=Freq/sum(Freq))
...
g19.6 <- datos.grafico %>% 
  ...
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  ggtitle('Sintomatología Depresiva, según Días de Cuarentena Acumulada')
...
g19.6 <- datos.grafico %>% 
  ...
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  ggtitle('Días de Cuarentena Acumulada, según Situación Ocupacional')
