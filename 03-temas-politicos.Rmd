...
tip_par <- tip_par[c("idencuesta", "class_part_4", "par_mov_w01", "par_mov_w02", "par_mov_w03", "par_mov_w04", "par_mov_w05", 'c11_w01', 'c11_w03', 'c43_w05')]
...
tab1 <- tip_par %>% 
  group_by(class_part_4, c11_w01) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 1) %>% 
  rename(Clase = class_part_4, 
         Participa = c11_w01)

tab2 <- tip_par %>% 
  group_by(class_part_4, c11_w03) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 2) %>% 
  rename(Clase = class_part_4, 
...
tab4 <- tip_par %>% 
  group_by(class_part_4, par_mov_w01) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 4) %>% 
  rename(Clase = class_part_4, 
...
tab5 <- tip_par %>% 
  group_by(class_part_4, par_mov_w02) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 5) %>% 
  rename(Clase = class_part_4, 
...
tab6 <- tip_par %>% 
  group_by(class_part_4, par_mov_w03) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 6) %>% 
  rename(Clase = class_part_4, 
...
tab7 <- tip_par %>% 
  group_by(class_part_4, par_mov_w04) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 7) %>% 
  rename(Clase = class_part_4, 
...
tab8 <- tip_par %>% 
  group_by(class_part_4, par_mov_w05) %>%
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>% 
  mutate(var = 8) %>% 
  rename(Clase = class_part_4, 
...
tab_4$Clase  <- factor(tab_4$Clase, 
                       levels = 1:4,
                       labels = c("Desafectados\n(56.51%)", "Manifestantes puros\n(3%)", 
                                  "Institucionales\n(24.34%)", "Reactivos\n(16.16%)"))

tab_4$Participa  <- factor(tab_4$Participa, levels = 1:2, 
                           labels = c("No participa", "Participa"))

tab_4$var  <- factor(tab_4$var, levels = 1:8, 
...
tip_par$class_part_4  <- factor(tip_par$class_part_4, 
                                levels = 1:4, 
                                labels = c("Desafectados", "Manifestantes puros", "Institucionales", "Reactivos"))
...
tip_par %>%   
  ...
  ggplot(aes(x = prop, y = class_part_4, 
             fill = pos_id, label = scales::percent(prop, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
...
tip_par %>%   
  ...
  group_by(class_part_4, m0_sexo_w01) %>% 
  summarise(n = n()) %>% 
  mutate(freq = n / sum(n)) %>%
  ggplot(aes(x = freq, y = class_part_4, fill = m0_sexo_w01, 
...
tip_par %>%   
  ...
  mutate(m0_sexo_w01 = factor(car::recode(m0_sexo_w01, recodes = "1 = 1; 2 = 2"),
                         ...
                         labels = c("Hombre", "Mujer"))) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(m0_sexo_w01, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(x = prop, y = class_part_4, fill = m0_sexo_w01, 
             label = scales::percent(prop, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1)) +
...
tip_par <- tip_par[c("idencuesta", "class_part_4")]
tip_par <- merge(tip_par, elsoc_wide_2016_2021, by="idencuesta")
tip_par$class_part_4  <- factor(tip_par$class_part_4, levels = c(1,2,3,4), labels = c("Desafectados", "Manifestantes\npuros", "Institucionales", "Reactivos"))
...
tip_par %>%   
  ...
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(pos_id, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(x = prop, y = class_part_4, fill = pos_id, 
             label = scales::percent(prop, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
  scale_x_continuous(labels = scales::percent) +
...
tip_par %>%   
  ...
  mutate(freq = n / sum(n)) %>%
  filter(!is.na(pos_id)) %>%
  ggplot(aes(x = freq, y = class_part_4, 
           fill = pos_id, label = scales::percent(freq, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1)) +
  guides(fill = guide_legend(reverse = TRUE)) +
...
tip_par <- tip_par[c("idencuesta", "class_part_4")]
tip_par <- merge(tip_par, elsoc_wide_2016_2021, by="idencuesta")
tip_par$class_part_4  <- factor(tip_par$class_part_4, levels = 1:4,
                                labels = c("Desafectados", "Manifestantes\npuros", "Institucionales", "Reactivos"))
...
tip_par %>%   
  ...
  filter(!is.na(pos_id)) %>%
  ggplot(aes(x = prop, y = class_part_4, 
             fill = fct_rev(pos_id), label = scales::percent(prop, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
  scale_x_continuous(labels = scales::percent) +
  guides(fill = guide_legend(reverse = TRUE)) +
...
tip_par <- tip_par[c("idencuesta", "class_part_4")]
tip_par <- merge(tip_par, elsoc_wide_2016_2021, by="idencuesta")
tip_par$class_part_4  <- factor(tip_par$class_part_4, levels = c(1,2,3,4), 
                                labels = c("Desafectados", "Manifestantes\npuros", "Institucionales", "Reactivos"))
