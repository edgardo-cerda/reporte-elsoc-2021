...
```{r, fig.cap = 'Porcentaje que presenta síntomas de depresión moderada a severa de forma reiterada, según número de ocurrencias' }
...
```{r, fig.cap = 'Porcentaje que presenta síntomas de depresión moderada a severa de forma reiterada, según número de ocurrencias y sexo' }
...
```{r, fig.cap = 'Cargas de sintomatología depresiva, según tramo de edad (en 2016)'}
...
elsoc_long_2016_2021 %>% 
  ...
  filter(tipo_atricion == 1 & ola %in% c(1, 3, 5),
         !is_nsnr(m43, m63_02_w05, m63_03_w05)) %>% 
  ...
  prop(m43 %in% 4:5, by = c(ola, retiros), na.rm = TRUE, vartype = 'cv') %>% 
  as_label(ola, retiros) %>% 
  ggplot(aes(y = prop, x = ola, group = retiros, color = retiros,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_line() + 
  geom_point() + 
  ...
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_color_viridis_d(end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())
```
...
elsoc_long_2016_2021 %>% 
  left_join(elsoc_wide_2016_2021 %>% 
              select(idencuesta, m63_02_w05, m63_03_w05), by = 'idencuesta') %>% 
  filter(tipo_atricion == 1 & ola %in% c(1, 3, 5),
         !is_nsnr(m44, m63_02_w05, m63_03_w05)) %>% 
  mutate(retiros = factor(case_when(m63_02_w05 == 2 ~ 1,
                                    m63_02_w05 == 1 & m63_03_w05 == 2 ~ 2,
                                    m63_02_w05 == 1 & m63_03_w05 == 1 ~ 3),
                          levels = 1:3, labels = c('No realizó ningún\nretiro',
  ...
  prop(m44 %in% 1, by = c(ola, retiros), na.rm = TRUE, vartype = 'cv') %>% 
  as_label(ola, retiros) %>% 
  ggplot(aes(y = prop, x = ola, group = retiros, color = retiros,
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
...
elsoc_long_2016_2021 %>% 
  ...
  as.data.frame() %>% 
  mutate(phq9 = (s11_01 + s11_02 + s11_03 + s11_04 + s11_05 + s11_06 + s11_07 + s11_08 + s11_09),
         m43 = factor(car::recode(m43, "1 = 1; 2:3 = 2; 4:5 = 3"),
                      levels = 1:3,
                      labels = c('Nada sobrecargado',
                                 'No muy o algo sobrecargado',
