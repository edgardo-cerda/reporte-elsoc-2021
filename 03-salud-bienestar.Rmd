...
```{r, fig.cap = 'Cargas de sintomatología depresiva, según deciles de ingreso (en 2021)'}

elsoc_depr_lca %>% 
  left_join(elsoc_clase %>% filter(ola == 5)) %>%  
  filter(tipo_atricion == 1, !is.na(decil)) %>% 
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(class_depr, by = decil, na.rm = TRUE) %>% 
  sjlabelled::as_label(class_depr, decil) %>%
  ggplot(aes(x = prop, y = decil, 
                   fill = fct_rev(class_depr),
              label = scales::percent(prop, accuracy = .1))) +
  theme_bw() +   
  geom_col() +
  geom_text(position = position_stack(vjust = .5), size = 3,
            color = rep(c('black', 'black', 'white', 'white'), 10)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_x_continuous(labels = scales::percent) +
  scale_fill_viridis_d(begin = .1, end = .8, direction = 1, option = 'viridis') +
