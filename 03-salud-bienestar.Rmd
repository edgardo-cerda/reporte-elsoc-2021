...
La figura 3.5 debe presentarse antes que la figura 3.4
...
```{r depre-wave-quintil, fig.cap = 'Porcentaje que presenta síntomas de depresión moderados a severos, según sexo y ola'}
...
elsoc_long_2016_2021 %>% 
  ...
  prop(phq9 >= 10 , by = c(ola, educ), na.rm = TRUE) %>% 
  sjlabelled::as_label(ola, educ) %>% 
  ggplot(aes(y = prop, x = ola, fill = educ, 
             label = scales::percent(prop, accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = 'dodge2') +
  scale_y_continuous(labels = scales::percent, limits = 0:1) +
...
elsoc_long_2016_2021 %>% 
  ...
  prop(phq9 >= 10 , by = c(ola, quintil), na.rm = TRUE) %>% 
  sjlabelled::as_label(ola, quintil) %>% 
  ggplot(aes(y = prop, x = ola, fill = quintil, 
             label = scales::percent(prop, accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = 'dodge2') +
  scale_y_continuous(labels = scales::percent, limits = 0:1) +
...
Interesante, pero difícil de interpretar. Dada su complejidad, mejor no presentar en reporte. 
...
Interesante mayor exposición de ciertas categorías ocupacionales, en particular trabajadores manuales. Sin embargo, la relación teórica entre categoría ocupacional y tener algún conocido diagnosticado no es tan evidente (sacaríamos columnas de la derecha). 
...
Hace sentido la diferencia de cumplimiento de distanciamiento social entre Q1 y Q5. Sin embargo, podría ser más pertinente utilizar nivel educacional como criterio de estratificación. 
