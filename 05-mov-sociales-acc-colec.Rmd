...
```{r}
datos.1.13 <- data.frame((svytable(~conf_carab + fecha_entr + ola, elsoc_diseno, round = F))) %>%
  group_by(fecha_entr) %>% 
  mutate(porcentaje=Freq/sum(Freq)) %>% 
  filter(ola==2021 & Freq>0 & conf_carab=="Nada y Poca")

datos.1.13$fecha_entr<-as.Date(datos.1.13$fecha_entr)

g1.13 <- datos.1.13 %>% 
  ggplot(aes(y = porcentaje, x = fecha_entr, 
             label = as.character(scales::percent(porcentaje, accuracy = .1)))) + 
  theme_bw() + 
  geom_col(position = 'Stack') +
  scale_y_continuous(labels = scales::percent,
