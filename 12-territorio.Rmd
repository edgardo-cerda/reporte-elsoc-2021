...
datos.8.5 <- elsoc_long_2016_2021 %>% 
  ...
  prop(x = barrio_confli_rec, by = c(ola, quintil), na.rm = TRUE) %>% 
  filter(barrio_confli_rec == 'Muchas veces o siempre')
...
## Cartografía regional del conflicto barrial
...
comunas_rm <- merge(comunas_rm,elsoc_comuna_rm, by.x="codigo_comuna",by.y="comuna_cod",all.x=TRUE,sort=F)
...
```{r confli-region, fig.align='center', fig.cap='Frecuencia promedio de problemas con vecinos, message=FALSE, warning=FALSE}
...
datos.8.8 <- elsoc_long_2016_2021 %>% 
  ...
  mutate(quintil = factor(ntile(-desc(ing_pc), 5), levels = c(1,2,3,4,5),
         labels = c('Q1', 'Q2', 'Q3', 'Q4', 'Q5'))) %>% 
  prop(x = barrio_conf, by = c(ola, quintil), na.rm = TRUE) %>% 
  filter(barrio_conf == 'Bastante o mucho')
