...
datos.g14.1 <- 
  data.frame((svytable(~amenaza_realista + ola + migrantes, elsoc_diseno_m1, round = F))) %>%
  group_by(ola) %>% 
  mutate(porcentaje = Freq/sum(Freq))

g14.1 <- ggplot(datos.g14.1, aes(y = porcentaje, x = ola, color = migrantes, group = amenaza_realista,
