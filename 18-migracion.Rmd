...
#Paso 1: crear dataset ponderado
datos.am_real <- data.frame((svytable(~migrantes + ola + muestra + amenaza_realista, elsoc_diseno, round = F))) %>%
  group_by(ola, muestra) %>% mutate(porcentaje=Freq/sum(Freq)) %>% na.omit()

#Paso 1.2: filtrar el subset manteniendo "De acuerdo o totalmente de acuerdo"
subset.g14.1 <- droplevels(subset(datos.am_real, amenaza_realista == 'De acuerdo o totalmente de acuerdo')) %>% 
  dplyr::filter(porcentaje > 0.00000000)

g14.1 <- 
  ggplot(subset.g14.1, aes(y = porcentaje, x = ola, color = migrantes, group = migrantes,
  ...
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_color_viridis_d(begin = .33, end = .99, direction = 1, option = 'viridis') +
...
#Paso 1: crear dataset ponderado
datos.am_sim <- data.frame((svytable(~migrantes + ola + muestra + amenaza_simbolica, elsoc_diseno, round = F))) %>%
  group_by(ola, muestra) %>% mutate(porcentaje=Freq/sum(Freq)) %>% na.omit()

#Paso 1.2: filtrar el subset manteniendo "De acuerdo o totalmente de acuerdo"
subset.g14.2 <- droplevels(subset(datos.am_sim, amenaza_simbolica == 'De acuerdo o totalmente de acuerdo')) %>% 
  dplyr::filter(porcentaje > 0.00000000)

g14.2 <- 
  ggplot(subset.g14.2, aes(y = porcentaje, x = ola, color = migrantes, group = migrantes,
                           label = as.character(scales::percent(porcentaje, accuracy = .1)))) +
  theme_bw() +   
  geom_line(size = 1) +
  geom_point(size = 1.8) +
