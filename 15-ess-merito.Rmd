...
elsoc_panel_m1 <- elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & muestra == 1 & ola %in% c(1,4,5) & !d01_01 %in% c(-888, -999)) %>% 
  mutate(clase_sub = factor(car::recode(d01_01, "c(0,1,2,3)=1; c(4, 5)=2; c(6,7,8,9,10)=3;else=NA"), 
                            levels = c(1,2,3), labels = c("Baja y media baja", "Media", "Alta y media alta"))) %>%
  sjlabelled::as_label(ola)
...
datos.11.2 <- data.frame((svytable(~clase_sub + ola + idencuesta, elsoc_diseno, round = F))) %>% 
  group_by(ola) %>% mutate(prop=Freq/sum(Freq)) %>%
  filter(Freq>0, ola == '2016'| ola == '2019' | ola == '2021') %>%
  drop_na()

etiquetas.11.2 <- data.frame((svytable(~clase_sub + ola, elsoc_diseno, round = F))) %>% 
...
elsoc_panel_m1 <- elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & muestra == 1 & ola %in% c(1,2,3,4,5) & 
           !c18_09 %in% c(-888, -999) & !c18_10 %in% c(-888, -999) & !d05_02 %in% c(-888, -999) & !d05_04 %in% c(-888, -999)) %>%
