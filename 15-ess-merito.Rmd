...
datos.g11.1 <- 
  data.frame((svytable(~clase.sub + ola, elsoc_diseno, round = F))) %>%
  group_by(ola) %>% 
...
datos.g11.4 <- 
  data.frame((svytable(~clase.sub.hijos + clase.sub + ola, elsoc_diseno, round = F))) %>%
...
<!-- ### 11.X  Perpectiva de movilidad social, según ola.  -->
```{r mov-soc-rec, fig.align='center',fig.cap='Perpectiva de movilidad social, según ola'}
datos.g11.X <- 
  data.frame((svytable(~mov_interg_rec + ola, elsoc_diseno, round = F))) %>%
  group_by(ola) %>% 
  mutate(porcentaje = Freq/sum(Freq))

g11.X <- 
  datos.g11.X %>% 
  ggplot(aes(y = porcentaje, x = ola, fill = mov_interg_rec, 
             label = as.character(scales::percent(porcentaje, accuracy = .1)))) + 
  theme_bw() + 
  geom_col(position = 'Stack') +
  scale_y_continuous(labels = scales::percent,
...
g11.X
```
<!-- ### 11.Y  Estatus subjetivo y tipo de movilidad social, nivel promedio según ola.  -->
...
datos.g11.Y$m_mov_social <- with(datos.g11.Y, case_when(
...
g11.Y <- 
  ggplot(datos.g11.Y, aes(y = m_mov_social, x = ola, group = mov_social, color = mov_social,
                              label = m_mov_social)) +
  theme_bw() +
  geom_line(size = 1) +
  geom_point(size = 1.8) +
