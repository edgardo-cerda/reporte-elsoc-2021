...
```{r confianzaveci-sexo, fig.align='center',fig.cap='Evolución de confianza en los vecinos según sexo'}
...
tab1<-tab %>% group_by(m0_sexo_w01, t01_w01) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 1) %>% rename(Confía=t01_w01) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(m0_sexo_w01)) %>% filter(Confía==1)
tab2<-tab %>% group_by(m0_sexo_w01, t01_w02) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 2) %>% rename(Confía=t01_w02) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(m0_sexo_w01)) %>% filter(Confía==1)
tab3<-tab %>% group_by(m0_sexo_w01, t01_w03) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 3) %>% rename(Confía=t01_w03) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(m0_sexo_w01)) %>% filter(Confía==1)
tab4<-tab %>% group_by(m0_sexo_w01, t01_w04) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 4) %>% rename(Confía=t01_w04) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(m0_sexo_w01)) %>% filter(Confía==1)
tab5<-tab %>% group_by(m0_sexo_w01, t01_w05) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 5) %>% rename(Confía=t01_w05) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(m0_sexo_w01)) %>% filter(Confía==1)
...
tab %>%
  ggplot(aes(x=var, y=freq, color=m0_sexo_w01)) +
  geom_line() +
  geom_point() +
  labs(y="", x="",
       title = "Evolución de confianza en los vecinos según sexo") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  scale_x_continuous(breaks = c(1,2,3,4,5), labels=c("2016", "2017", "2018", "2019", "2021")) +
...
```{r confianzaveci-zona, fig.align='center',fig.cap='Evolución de confianza en los vecinos según zona'}

tab<-territorial %>%   
  filter(!is_nsnr(estrato_w05, t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("estrato_w05", "t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>%
  mutate(t01_w01 = factor(car::recode(t01_w01, recodes = "4:5 = 1; 1:3 = 0"))) %>%
  mutate(t01_w02 = factor(car::recode(t01_w02, recodes = "4:5 = 1; 1:3 = 0"))) %>%
...
tab1<-tab %>% group_by(estrato_w05, t01_w01) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 1) %>% rename(Confía=t01_w01) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(estrato_w05)) %>% filter(Confía==1) %>%
  filter(estrato_w05=="Gran Santiago" | estrato_w05=="Gran Concepción" | estrato_w05=="Ciudades pequeñas")
tab2<-tab %>% group_by(estrato_w05, t01_w02) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 2) %>% rename(Confía=t01_w02) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(estrato_w05)) %>% filter(Confía==1) %>%
  filter(estrato_w05=="Gran Santiago" | estrato_w05=="Gran Concepción" | estrato_w05=="Ciudades pequeñas")
tab3<-tab %>% group_by(estrato_w05, t01_w03) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 3) %>% rename(Confía=t01_w03) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(estrato_w05)) %>% filter(Confía==1) %>%
  filter(estrato_w05=="Gran Santiago" | estrato_w05=="Gran Concepción" | estrato_w05=="Ciudades pequeñas")
tab4<-tab %>% group_by(estrato_w05, t01_w04) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 4) %>% rename(Confía=t01_w04) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(estrato_w05)) %>% filter(Confía==1) %>%
  filter(estrato_w05=="Gran Santiago" | estrato_w05=="Gran Concepción" | estrato_w05=="Ciudades pequeñas")
tab5<-tab %>% group_by(estrato_w05, t01_w05) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 5) %>% rename(Confía=t01_w05) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(estrato_w05)) %>% filter(Confía==1) %>%
  filter(estrato_w05=="Gran Santiago" | estrato_w05=="Gran Concepción" | estrato_w05=="Ciudades pequeñas")
...
tab %>%
  ...
  geom_point() +
  labs(y="", x="",
       title = "Evolución de confianza en los vecinos según zona") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  scale_x_continuous(breaks = c(1,2,3,4,5), labels=c("2016", "2017", "2018", "2019", "2021")) +
  scale_color_viridis_d(begin = .33, end = .88, option = 'viridis', name  =" ") +
  theme_minimal( ) + theme(legend.position = "bottom", plot.title = element_text(size=12))
...
```{r confianzaveci-quintil, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de ingresos'}
...
tab1<-tab %>% group_by(quintil, t01_w01) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 1) %>% rename(Confía=t01_w01) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(quintil)) %>% filter(Confía==1) %>%
  filter(quintil=="Q1" | quintil=="Q5")
tab2<-tab %>% group_by(quintil, t01_w02) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 2) %>% rename(Confía=t01_w02) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(quintil)) %>% filter(Confía==1) %>%
  filter(quintil=="Q1" | quintil=="Q5")
tab3<-tab %>% group_by(quintil, t01_w03) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 3) %>% rename(Confía=t01_w03) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(quintil)) %>% filter(Confía==1) %>%
  filter(quintil=="Q1" | quintil=="Q5")
tab4<-tab %>% group_by(quintil, t01_w04) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 4) %>% rename(Confía=t01_w04) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(quintil)) %>% filter(Confía==1) %>%
  filter(quintil=="Q1" | quintil=="Q5")
tab5<-tab %>% group_by(quintil, t01_w05) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  mutate(var = 5) %>% rename(Confía=t01_w05) %>% filter(!is.na(Confía)) %>%
  filter(!is.na(quintil)) %>% filter(Confía==1) %>%
  filter(quintil=="Q1" | quintil=="Q5")
tab<- rbind(tab1, tab2, tab3, tab4, tab5)
...
tab %>%
  ...
  geom_point() +
  labs(y="", x="",
       title = "Evolución de confianza en los vecinos según quintil de ingresos") +
  scale_y_continuous(labels = scales::percent, limits = c(0, 1)) +
  scale_x_continuous(breaks = c(1,2,3,4,5), labels=c("2016", "2017", "2018", "2019", "2021")) +
  scale_color_viridis_d(begin = .33, end = .88, option = 'viridis', name  =" ") +
  theme_minimal( ) + theme(legend.position = "bottom", plot.title = element_text(size=12))
...
```{r confianzaveci-quintilpueblos, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de pueblos originarios'}

territorial$porig[territorial$porig < 0] <- NA
territorial$hbtes[territorial$hbtes < 0] <- NA
territorial$p_orig <- territorial$porig/territorial$hbtes

tab<-territorial %>%   
  filter(!is_nsnr(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1)  %>%
  mutate(quintil = xtile(p_orig, n = 5, wt = ponderador02_w05)) %>%
         mutate(quintil = factor(quintil, levels = 1:5, labels = glue::glue('Q{1:5}')))  %>%
  rowwise() %>%
          mutate(confianza = mean(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05, na.rm = TRUE)) %>%
          mutate(Confía= factor(confianza >= 4,
...
tab %>% ggplot(aes(y = freq, x = quintil, label = scales::percent(freq, accuracy = .1))) +
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y="", x="",
       title = "Confianza en los vecinos según quintil de proporción de pueblos originarios en zona censal") +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  theme(legend.position = 'top',
        legend.title = element_blank())
...
```{r confianzaveci-quintilinm, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de inmigrantes'}
...
```{r confianzaveci-quintilinmlat, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de inmigrantes latinos'}

territorial$argentino[territorial$argentino < 0] <- NA
territorial$colombiano[territorial$colombiano < 0] <- NA
territorial$peruano[territorial$peruano < 0] <- NA
territorial$boliviano[territorial$boliviano < 0] <- NA
territorial$venezolano[territorial$venezonlano< 0] <- NA
territorial$haitiano[territorial$haitiano < 0] <- NA
territorial$inmig_lac <- territorial$argentino+territorial$colombiano+territorial$peruano+territorial$boliviano+territorial$venezolano+territorial$haitiano
territorial$p_inmig_lac <- territorial$inmig_lac/territorial$hbtes

tab<-territorial %>%   
  filter(!is_nsnr(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1)  %>%
  mutate(quintil = xtile(p_inmig_lac, n = 5, wt = ponderador02_w05)) %>%
         mutate(quintil = factor(quintil, levels = 1:5, labels = glue::glue('Q{1:5}')))  %>%
  rowwise() %>%
          mutate(confianza = mean(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05, na.rm = TRUE)) %>%
          mutate(Confía= factor(confianza >= 4,
...
tab %>% ggplot(aes(y = freq, x = quintil, label = scales::percent(freq, accuracy = .1))) +
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y="", x="",
       title = "Confianza en los vecinos según quintil de proporción de inmigrantes latinoamericanos en zona censal") +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  theme(legend.position = 'top',
        legend.title = element_blank())
...
```{r confianzaveci-quintilinmnort, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de inmigrantes norteamericanos y europeos'}

territorial$europeo[territorial$europeo < 0] <- NA
territorial$norteamericano[territorial$norteamericano < 0] <- NA
territorial$inmig_eu <- territorial$europeo+territorial$norteamericano
territorial$p_inmig_eu <- territorial$inmig_eu/territorial$hbtes

tab<-territorial %>%   
  filter(!is_nsnr(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1)  %>%
  mutate(quintil = xtile(p_inmig_eu, n = 5, wt = ponderador02_w05)) %>%
         mutate(quintil = factor(quintil, levels = 1:5, labels = glue::glue('Q{1:5}')))  %>%
  rowwise() %>%
          mutate(confianza = mean(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05, na.rm = TRUE)) %>%
          mutate(Confía= factor(confianza >= 4,
...
tab %>% ggplot(aes(y = freq, x = quintil, label = scales::percent(freq, accuracy = .1))) +
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y="", x="",
       title = "Confianza en los vecinos según quintil de proporción de inmigrantes norteamericanos y europeos en zona censal") +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  theme(legend.position = 'top',
        legend.title = element_blank())
...
```{r confianzaveci-quintilesc, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de  escolaridad'}
...
```{r confianzaveci-quintilescjefe, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de  escolaridad del jefe de hogar'}

territorial$jhescm[territorial$jhescm < 0] <- NA

tab<-territorial %>%   
  filter(!is_nsnr(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1)  %>%
  mutate(quintil = xtile(jhescm, n = 5, wt = ponderador02_w05)) %>%
         mutate(quintil = factor(quintil, levels = 1:5, labels = glue::glue('Q{1:5}')))  %>%
  rowwise() %>%
          mutate(confianza = mean(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05, na.rm = TRUE)) %>%
          mutate(Confía= factor(confianza >= 4,
...
tab %>% ggplot(aes(y = freq, x = quintil, label = scales::percent(freq, accuracy = .1))) +
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y="", x="",
       title = "Confianza en los vecinos según quintil de escolaridad del jefe de hogar en zona censal") +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  theme(legend.position = 'top',
        legend.title = element_blank())
...
```{r confianzaveci-quintilescjefevar, fig.align='center',fig.cap='Evolución de confianza en los vecinos según quintil de  varianza de escolaridad del jefe de hogar'}

territorial$jhescsd[territorial$jhessd < 0] <- NA
territorial$jhescsd2<-territorial$jhescsd*territorial$jhescsd

tab<-territorial %>%   
  filter(!is_nsnr(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05))  %>%
  drop_na(c("t01_w01", "t01_w02", "t01_w03", "t01_w04", "t01_w05")) %>%
  filter(tipo_atricion==1)  %>%
  mutate(quintil = xtile(jhescsd2, n = 5, wt = ponderador02_w05)) %>%
         mutate(quintil = factor(quintil, levels = 1:5, labels = glue::glue('Q{1:5}')))  %>%
  rowwise() %>%
          mutate(confianza = mean(t01_w01, t01_w02, t01_w03, t01_w04, t01_w05, na.rm = TRUE)) %>%
          mutate(Confía= factor(confianza >= 4,
...
tab %>% ggplot(aes(y = freq, x = quintil, label = scales::percent(freq, accuracy = .1))) +
  ...
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  labs(y="", x="",
       title = "Confianza en los vecinos según quintil de varianza de escolaridad del jefe de hogar en zona censal") +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  theme(legend.position = 'top',
        legend.title = element_blank())
