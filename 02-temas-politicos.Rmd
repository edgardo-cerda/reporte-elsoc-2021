...
tab_4$Clase  <- factor(tab_4$Clase, levels = c(1,2,3,4), labels = c("Desafectados\n(56.51%)", "Manifestantes puros\n(3%)", "Institucionales\n(24.34%)", "Reactivos\n(16.16%)"))
tab_4$Participa  <- factor(tab_4$Participa, levels = c(1,2), labels = c("No participa", "Participa"))
tab_4$var  <- factor(tab_4$var, levels = c(1,2,3,4,5,6,7,8), labels = c("Elección 2013", "Elección 2017", "Plebiscito 2020", "Marcha 2016", "Marcha 2017", "Marcha 2018", "Marcha 2019", "Marcha 2021"))
...
tip_par %>%   
  ...
  filter(!is.na(pos_id)) %>%
  ggplot(aes(x = freq, y = class_part_4, 
             fill = fct_rev(pos_id), label = scales::percent(freq, .1))) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y = NULL, x = NULL, title = NULL, fill = NULL) +
  scale_x_continuous(labels = scales::percent, limits = c(0, 1)) +
  guides(fill = guide_legend(reverse = TRUE)) +
