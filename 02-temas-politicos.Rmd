...
group_by(class_part_4, pos_id) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  filter(!is.na(pos_id)) %>% 
  ggplot(aes(y = freq, fill = class_part_4, x = pos_id)) +
  theme_bw() + 
  geom_col(position = 'dodge2') + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), limits=c(0, 1)) +
     labs(y="Proporción", x="Posición política",
...
group_by(class_part_4, pos_id) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ...
  geom_col(position = 'dodge2') + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), limits=c(0, 1)) +
     labs(y="Proporción", x="Tramo etario",
...
group_by(class_part_4, pos_id) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ...
  geom_col(position = 'dodge2') + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), limits=c(0, 1)) +
     labs(y="Proporción", x="Interés en la política",
...
group_by(class_part_4, pos_id) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
  ...
  geom_col(position = 'dodge2') + 
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), limits=c(0, 1)) +
     labs(y="Proporción", x="Satisfacción con la democracia",
