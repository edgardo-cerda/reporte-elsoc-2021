...
Fue modificada incorporando sólo marchas (c08_02). Además se categorizó como 1 cuando participa en marchas a veces, frecuente o muy frecuentemente y como 0 cuando participa nunca o casi nunca. Junto a lo anterior se probó eliminar las olas 1 y 5 para que tanto participación electoral como en protestas tengan 3 variables cada una, pesando lo mismo en el modelo. Los resultados de esto último eran confusos por lo que se definió mantener las 5 olas de participación en protestas. Los resultados descriptivos para cada clase están con factores de expansión incorporados.
...
# write.csv(tip_par, file="participacion_tipos3.csv", row.names = F, fileEncoding = "UTF-8")
...
tab_3$Clase  <- factor(tab_3$Clase, levels = c(1,2,3), labels = c("(60.6%)", "(14.25%)", "(25.15%)"))
...
tab_4$Clase  <- factor(tab_4$Clase, levels = c(1,2,3,4), labels = c("Desafectados (56.51%)", "Movilizados (3%)", "Institucionales (24.34%)", "Reclutados (16.16%)"))
...
tab_5$Clase  <- factor(tab_5$Clase, levels = c(1,2,3,4,5), labels = c("(3.2%)", "(55.62%)", "(2.86%)", "(14.72%)", "(23.59%)"))
...
```{r tipos-democracia, fig.cap = 'Legitimidad democrática por tipos de participación'}

tip_par <- read.csv(file.path('inputs', 'participacion_tipos3.csv'), sep=",", encoding="UTF-8")
tip_par <- tip_par[c("idencuesta", "class_part_4")]
tip_par <- merge(tip_par, elsoc_wide_2016_2021, by="idencuesta")
tip_par$class_part_4  <- factor(tip_par$class_part_4, levels = c(1,2,3,4), labels = c("Desafectados", "Movilizados", "Institucionales", "Reclutados"))
...
```{r tipos-posicion-política, fig.cap = 'Posición política por tipos de participación'}
...
tip_par %>%   
  ...
  filter(tipo_atricion == 1) %>%
  mutate(pos_id = factor(car::recode(c15_w01, recodes = "0:4 = 1; 5 = 2; 6:10 = 3; 11:12 = 4"),
                         levels = 1:4,
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(pos_id, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = class_part_4, fill = pos_id,
             label = scales::percent(prop, .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') + 
  ...
  scale_y_continuous(labels = scales::percent, 
                     limits = c(0, 1)) +
     labs(y = "Proporción", x = "Tipo de participación",
       title = "Identificación política por tipo de participación") +
...
```{r tipos-sexo, fig.cap = 'Sexo por tipos de participación'}
...
tip_par %>%   
  ...
  filter(tipo_atricion==1) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  mutate(m0_sexo_w05 = factor(m0_sexo_w05, levels = 1:2,
                              labels = c('Hombre', 'Mujer'))) %>% 
  prop(m0_sexo_w05, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = class_part_4, fill = m0_sexo_w05,
             label = scales::percent(prop, .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5,
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), 
                     limits=c(0, 1)) +
     labs(y="Proporción", x="Tipo de participación",
       title = "Sexo por tipo de participación") +
  scale_fill_viridis_d(begin = .33, end = .66, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
        legend.title = element_blank())
...
```{r tipos-edad, fig.cap = 'Edad por tipos de participación'}
...
tip_par %>%   
  ...
  mutate(edadt = factor(car::recode(m0_edad_w05, "18:29 = 1; 30:49 = 2; 50:64 = 3; 65:150 = 4"),
                        ...
                        labels = c('18-29', '30-49', '50-64', '65 o más'))) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(edadt, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = class_part_4, fill = edadt,
             label = scales::percent(prop, .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5,
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), 
                     limits=c(0, 1)) +
     labs(y="Proporción", x="Tipo de participación",
       title = "Edad por tipo de participación") +
  scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
        legend.title = element_blank())
...
```{r tipos-interes, fig.cap = 'Interés por tipos de participación'}
...
tip_par %>%   
  ...
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(interes, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = class_part_4, fill = interes,
             label = scales::percent(prop, .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5,
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), 
                     limits=c(0, 1)) +
     labs(y="Proporción", x="Tipo de participación",
       title = "Interés en la política por tipo de participación") +
  scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
        legend.title = element_blank())
...
```{r tipos-satisfaccion, fig.cap = 'Satisfacción con la democracia por tipos de participación'}
...
tip_par %>%   
  ...
  filter(tipo_atricion==1) %>%
  mutate(satisfaccion = factor(car::recode(c01_w05, "1:2 = 1; 3 = 2; 4:5 = 3"),
                        levels = 1:3,
                        labels = c('Nada o poco satisfecho', 'Algo satisfecho', 'Bastante o muy satisfecho'))) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(satisfaccion, by = class_part_4, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = class_part_4, fill = satisfaccion,
             label = scales::percent(prop, .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5,
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), 
                     limits=c(0, 1)) +
     labs(y="Proporción", x="Tipo de participación",
       title = "Satisfacción con la democracia por tipo de participación") +
  scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
        legend.title = element_blank())
...
group_by(pos_id, class_part_4) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
    filter(!is.na(pos_id)) %>% 
ggplot(aes(x = freq, y = pos_id, fill = class_part_4)) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y="Indentificación política", x="Proporción",
       title = "Identificación política por tipo de participación", fill = "") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 1)) +
  theme_bw() + scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
...
ggplot(aes(x = freq, y = pos_id, fill = class_part_4)) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y="Sexo", x="Proporción",
       title = "Sexo por tipo de participación", fill = "") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 1)) +
  theme_bw() + scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
...
ggplot(aes(x = freq, y = pos_id, fill = class_part_4)) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y="Tramo etario", x="Proporción",
       title = "Edad por tipo de participación", fill = "") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 1)) +
  theme_bw() + scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
...
ggplot(aes(x = freq, y = pos_id, fill = class_part_4)) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y="Interés en la política", x="Proporción",
       title = "Interés en la política por tipo de participación", fill = "") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 1)) +
  theme_bw() + scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
...
tip_par %>%   
  ...
  drop_na(c("class_part_4", "c01_w05")) %>%
  filter(tipo_atricion==1) %>%
  mutate(pos_id = factor(car::recode(c01_w01, "1:2 = 1; 3 = 2; 4:5 = 3"),
                        levels = 1:3,
                        labels = c('Nada o poco satisfecho', 'Algo satisfecho', 'Bastante o muy satisfecho'))) %>%
group_by(pos_id, class_part_4) %>% summarise (n = n()) %>% mutate(freq = n / sum(n)) %>%
   filter(!is.na(pos_id)) %>%
ggplot(aes(x = freq, y = pos_id, fill = class_part_4)) + 
  geom_bar(stat = 'identity', position = 'stack') +
  labs(y="Satisfacción con la democracia", x="Proporción",
       title = "Satisfacción con la democracia por tipo de participación", fill = "") +
  scale_x_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 1)) +
  theme_bw() + scale_fill_viridis_d(end = .85, option = 'viridis') +
  theme(axis.text.x = element_text(size = rel(.9)),
        legend.position = 'top',
...
tab %>%
  ggplot(aes(x=var, y=freq, color=name)) +
  geom_line() +
  geom_point() +
  labs(y="Proporción",
       title = "Participación según sobrecarga en pandemia") +
  scale_y_continuous(breaks = scales::pretty_breaks(n = 10), limits=c(0, 1)) +
  scale_x_continuous(breaks = c(1,2,3), labels=c("Elección 2013", "Elección 2017", "Plebiscito")) +
...
tab %>%
  ggplot(aes(x=var, y=freq, color=optimista)) +
  geom_line() +
  geom_point() +
  labs(y="Proporción", x="Wave",
...
tab %>%
  ...
  scale_y_continuous(breaks = scales::pretty_breaks(n = 5), limits=c(0, 0.25)) +
  scale_x_continuous(breaks = c(1,2,3,4,5), labels=c("W1", "W2", "W3", "W4", "W5")) +
  scale_color_viridis_d(begin = .33, end = .88, option = 'viridis', name  =" ") +
  theme_minimal( ) + theme(legend.position = "bottom", plot.title = element_text(size=12))
rm(tab)
