...
elsoc_panel_m1 <- elsoc_long_2016_2021 %>% 
  ...
  mutate(pos_id = factor(pos_id, levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  mutate(missing = any(pos_id %in% c("NS/NR"))) %>% 
  filter(!missing) %>% 
  ungroup() %>%
...
```{r include=FALSE}
datos.2.3 <- elsoc_wide_2016_2021 %>%
mutate(ac_cuar = car::recode(c37_10_w05, recodes = "c(1,2)= 1;
                                      c(3)= 2; 
                                      c(4,5)= 3;
                                      else=NA", as.factor = TRUE)) %>%
  filter(tipo_atricion == 1 & muestra == 1 & ac_cuar %in% c(3)) %>% 
  survey_design_elsoc(weights = 'ponderador02_w05') %>%
  mutate(pos_id = car::recode(c15_w05, recodes = "c(0,1,2,3,4)=1; c(5)=2; c(6,7,8,9,10)=3; c(11,12, -999, -888)=4")) %>%
  mutate(pos_id = factor(pos_id, levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
    prop(pos_id, by = c(ac_cuar), na.rm = TRUE) %>% 
  sjlabelled::as_label(pos_id, ac_cuar)
...
```{r include=FALSE}
datos.2.4 <- elsoc_wide_2016_2021 %>%
  filter(tipo_atricion == 1 & muestra == 1 & !c37_09_w05 %in% c(-888, -999)) %>%
mutate(salud_ac = car::recode(c37_09_w05, recodes = "c(1,2)= 1;
                                      c(3)= 2; 
                                      c(4,5)= 3;
                                      else=NA", as.factor = TRUE)) %>%
  filter(salud_ac %in% c(3)) %>% 
  survey_design_elsoc(weights = 'ponderador02_w05') %>%
  mutate(pos_id = car::recode(c15_w05, recodes = "c(0,1,2,3,4)=1; c(5)=2; c(6,7,8,9,10)=3; c(11,12, -999, -888)=4")) %>%
  mutate(pos_id = factor(pos_id, levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
    prop(pos_id, by = c(salud_ac), na.rm = TRUE)
...
datos.2.6 <- elsoc_long_2016_2021 %>%
  filter(tipo_atricion == 1) %>%
  mutate(aborto = car::recode(c37_02, recodes = "c(1,2)= 1;
                                      c(3)= 2; 
                                      c(4,5)= 3;
                                      else=NA", as.factor = TRUE)) %>%
  mutate(pos_id = car::recode(c15, recodes = "c(0,1,2,3,4)=1; c(5)=2; c(6,7,8,9,10)=3; c(11,12, -999, -888)=4")) %>%
  filter(aborto %in% c(3)) %>% 
  mutate(pos_id = factor(pos_id, levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  prop(x = pos_id, by = c(aborto, ola), na.rm = TRUE) %>% 
  sjlabelled::as_label(pos_id, aborto)

g2.6 <-datos.2.6 %>% 
  ggplot(aes(y = prop, x = ola, fill = aborto, 
               label = as.character(scales::percent(prop, accuracy = .1)))) +
    theme_bw() + 
    geom_col(position= 'dodge2') +
    #escala del eje y en porcentajes del 0 al 100%
...
datos.2.7 <- elsoc_long_2016_2021 %>%
  ...
  sjlabelled::as_label(pos_id, cap_pension)
```
...
datos.2.8 <- elsoc_long_2016_2021 %>%
  ...
  sjlabelled::as_label(pos_id, rest_migrantes) %>%
  na.omit()
```
