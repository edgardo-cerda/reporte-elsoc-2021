
# Democracia y proceso constituyente

## Cambio constitucional


```{r servel-apruebo, fig.align='center', fig.cap='Voto retrospectivo Plebiscito Nueva Constitucion'}

elsoc_long_2016_2021 %>% 
  dplyr::filter(ola == 5 & tipo_atricion == 1 & !c44 %in% c(3, 4, -888, -999)& !is.na(c44)) %>% 
  prop(c44, na.rm = TRUE) %>% 
  sjlabelled::as_label(c44) %>%
  mutate(datos = 'Datos ELSOC') %>%
  add_row(c44 = c('Apruebo', 'Rechazo'), 
          prop = c(.7828, .2172), 
          datos = c('Datos servel', 'Datos servel')) %>%
  ggplot(aes(y = prop, x = c44, fill = datos, 
             label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .33, end = .66, direction = -1, option = 'viridis') +
  geom_text(vjust = -0.8,
            position = position_dodge(width = .9),
            size= 2.75)  + 
  theme(legend.position = 'top',
        legend.title = element_blank())

elsoc_long_2016_2021 %>% 
  dplyr::filter(ola == 5 & tipo_atricion == 1& !c43 %in% c(-888, -999)) %>% 
  prop(c43, na.rm = TRUE) %>% 
  as_label(c43) %>% 
  ggplot(aes(y = prop, x = c43, label = scales::percent(prop, accuracy = .1))) +
  theme_bw() + 
  geom_col(position = 'dodge2') +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .33, end = .66, direction = -1, option = 'viridis') +
  geom_text(vjust = -0.8,
            position = position_dodge(width = .9),
            size= 2.75)  + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```


```{r particip-elect-id, fig.align='center', fig.cap= 'Sí Participa en Elecciones según Identificación Política'}

elsoc_long_2016_2021 %>% 
  mutate(particip_electoral = ifelse(ola == 5, c43, c11),
         pos_id = factor(car::recode(c15, recodes = "0:4 = 1; 5 = 2; 6:10 = 3; 11:12 = 4"),
                         levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  filter(tipo_atricion == 1 & !particip_electoral %in% c(3, -888, -999) & 
           ola %in% c(1,3,5) & !is.na(pos_id)) %>% 
  prop(particip_electoral == 1, by = c(ola, pos_id), na.rm = TRUE) %>%
  sjlabelled::as_label(pos_id) %>% 
  mutate(ola = factor(ola, levels = c(1,3,5), labels = c('Elecciones Presidenciales\nde 2013', 'Elecciones Presidenciales\nde 2017', 'Plebiscito cambio\nconstitucional'))) %>% 
  ggplot(aes(y = prop, x = ola, color = pos_id, group = pos_id,
              label = scales::percent(prop, accuracy = .1))) +
  theme_bw() +   
  geom_line() +
  geom_point() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0,1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_color_viridis_d(end = .66, direction = 1, option = 'viridis') +
  ggrepel::geom_text_repel(size = 3, nudge_y = .025) + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```

```{r}

elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & ola %in% c(1, 3, 5) & 
           !c11 %in% c(3, -888, -999) & !c43 %in% c(3, -888, -999)) %>% 
  mutate(particip_electoral = ifelse(ola == 5, c43, c11),
         ola = factor(ola, levels = c(1,3,5), 
                      labels = c('Elecciones Presidenciales\nde 2013', 
                                 'Elecciones Presidenciales\nde 2017', 
                                 'Plebiscito cambio\nconstitucional'))) %>% 
  as_label(ola) %>% 
  survey_design_elsoc() %>%
  svytable(~particip_electoral + ola + idencuesta, design = .) %>% 
  as.data.frame() %>% 
  group_by(ola) %>% 
  mutate(prop = Freq/sum(Freq)) %>%
  filter(Freq > 0) %>% 
  ggplot(aes(y = prop, x = ola, fill = fct_rev(particip_electoral), 
             stratum = fct_rev(particip_electoral), alluvium = idencuesta)) +
  theme_bw() +
  ggalluvial::geom_flow(alpha = .66) + 
  ggalluvial::geom_stratum(linetype = 0, width = .55) +
  scale_y_continuous(labels = scales::percent) + 
  ylab(label = NULL) +
  xlab(label = NULL) + 
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  scale_fill_viridis_d(begin = .33, end = .66) +
  geom_text(data = function(x) { group_by(.data = x, ola, particip_electoral) %>% 
      summarise(prop = sum(prop, na.rm = TRUE), idencuesta = 1) %>% 
      ungroup()}, 
            aes(label = scales::percent(prop, accuracy = .1)),
            position = position_stack(vjust = .5),
            show.legend = FALSE,
            size = 3) + 
  guides(fill = guide_legend(reverse = TRUE)) + 
  ggtitle(label = 'Cambios en participación electoral, según ola del estudio')

```


```{r id-pol-voto, fig.align='center', fig.cap= "Voto Retrospectivo Plebicito, según Posición Ideológica"}

elsoc_long_2016_2021 %>% 
  filter(ola == 5 & tipo_atricion == 1 & !c44 %in% c(3,4, -888, -999) & !c15 %in% c(-888, -999)) %>% 
  mutate(pos_id = factor(car::recode(c15, recodes = "0:4 = 1; 5 = 2; 6:10 = 3; 11:12 = 4"),
                         levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  prop(c44, by = c(pos_id), na.rm = TRUE, vartype = NULL) %>% 
  sjlabelled::as_label(c44, pos_id) %>% 
  ggplot(aes(y = prop, x = pos_id, fill = c44, 
             label = as.character(scales::percent(prop, accuracy = .1)))) + 
  theme_bw() + 
  geom_col(position = "Stack") +
  scale_y_continuous(labels = scales::percent) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .33, end = .66, direction = -1, option = 'viridis') +
  geom_text(position = position_stack(vjust = .5),
            size= 2.75, color = rep.int(c('black', 'white'), 4)) + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```

```{r presi-voto-c44, fig.align='center', fig.cap= "Voto Retrospectivo Plebicito, según Posición Ideológica"}

elsoc_wide_2016_2021 %>% 
  dplyr::filter(tipo_atricion == 1 & 
                  !c11_w03 %in% c(-888, -999) &
                  !c43_w05 %in% c(-888, -999) &
                  !c44_w05 %in% c(-888, -999) & 
                  !c39_w03 %in% c(-888, -999)) %>%
  mutate(voto2017 = case_when(c11_w03 %in% 2:3 ~ 11,
                               TRUE ~ c39_w03),
         voto2017 = factor(car::recode(voto2017, '9:10 = 9'),
                           levels = c(1:9, 11),
                           labels = c('Goic', 'Kast', 'Piñera', 'Guillier', 'Sánchez', 'Enríquez-\nOminami', 'Artés', 'Navarro', 'Blanco\no Nulo', 'No votó')),
         voto2020 = case_when(c43_w05 %in% 2:3 ~ 5,
                              TRUE ~ c44_w05),
         voto2020 = factor(car::recode(voto2020, '3:4 = 3'),
                          levels = c(1:3, 5),
                          labels = c('Apruebo', 'Rechazo', 'Blanco\no Nulo', 'No votó'))) %>%
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(voto2020, by = voto2017, na.rm = TRUE) %>%
  dplyr::right_join(tidyr::expand(., voto2020, voto2017)) %>% 
  mutate(prop = ifelse(is.na(prop), 0, prop)) %>% 
  group_by(voto2017) %>% 
  mutate(orden = max(ifelse(voto2020 == 'Apruebo', prop, 0))) %>% 
  arrange(voto2017, voto2020) %>% 
  ggplot(aes(y = prop, x = reorder(voto2017, desc(orden)), fill = fct_rev(voto2020), 
             label = scales::percent(ifelse(prop < .02, NA, prop), accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = 'stack') +
  scale_y_continuous(labels = scales::percent) +
  ylab(label = NULL) +
  xlab(label = 'Voto en elección presidencial de 2017') +
  scale_fill_viridis_d(end = .7, direction = 1, option = 'viridis') +
  geom_text(position = position_stack(vjust = .5),
            size = 3,
            color = rep(c('black', 'white', 'white', 'white'), 10)) + 
  guides(fill = guide_legend(reverse = TRUE)) +
  theme(legend.position = 'top',
        legend.title = element_blank()) +
  ggtitle('Voto en plebiscito de 2020, según voto en elección presidencial de 2017')

```



```{r 2020-vs-2021, fig.align='center', fig.cap= "Participación Electoral 2021, según la Participación en 2017"}
datos.11.7 <- elsoc_wide_2016_2021 %>% 
  filter(tipo_atricion == 1 & !c43_w05 %in% c(3, -888, -999) & 
           !c11_w03 %in% c(3, -888, -999)) %>% 
  survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(c43_w05, by = c11_w03) %>% 
  sjlabelled::as_label(c43_w05, c11_w03)

datos.11.7 %>% 
  ggplot(aes(y = prop, x = c11_w03, fill = c43_w05, 
             label = scales::percent(prop, accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = "Stack") +
  scale_y_continuous(labels = scales::percent) + 
  ylab(label = NULL) +
  xlab(label = "Participación Electoral en 2017") +
  scale_fill_viridis_d(begin = .33, end = .66, direction = -1, option = 'viridis') + 
  geom_text(position = position_stack(vjust = .5),
            show.legend = FALSE,
            size = 2.75,
            color = rep(c('black', 'white'), 2)) + 
  theme(legend.position = 'top',
        legend.title = element_blank()) 
```

```{r cambio_participa}
#grafico y caracterización pendiente
elsoc_wide_2016_2021 %>% 
  filter(tipo_atricion == 1 & !c11_w03 %in% c(-888, -999, 3) & !c43_w05 %in% c(-888, -999, 3) &
           !c15_w03 %in% c(-888, -999)) %>% 
  mutate(cambio_participa = factor(case_when(c11_w03 == 1 & c43_w05 == 2 ~ 1,
                                             c11_w03 == 1 & c43_w05 == 1 ~ 2,
                                             c11_w03 == 2 & c43_w05 == 2 ~ 3,
                                             c11_w03 == 2 & c43_w05 == 1 ~ 4),
                                   labels = c('Se mantiene no votando', 'Cambia a votar',
                                              'Cambia a No Votar', 'Se mantiene votando')),
         pos_id_w03 = factor(car::recode(c15_w03, "0:4 = 1; 5 = 2; 6:10 = 3; 11:12 = 4"),
                         levels = c(1, 2, 3, 4),
                         labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>% 
  elsoc::survey_design_elsoc(weights = 'ponderador02_w05') %>% 
  prop(cambio_participa, by = pos_id_w03, na.rm = TRUE) %>% 
  ggplot(aes(y = prop, x = pos_id_w03, fill = cambio_participa, 
             label = scales::percent(prop, accuracy = .1))) + 
  theme_bw() + 
  geom_col(position = "Stack") +
  scale_y_continuous(labels = scales::percent) + 
  ylab(label = NULL) +
  xlab(label = "Posición ideológica en 2017") +
  scale_fill_viridis_d(begin = .33, end = .66, direction = -1, option = 'viridis') + 
  geom_text(position = position_stack(vjust = .5),
            show.legend = FALSE,
            size = 2.75,
            color = rep(c('black', 'black', 'white', 'white'), 4)) + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```

## Optimismo constitucional

```{r}
elsoc_long_2016_2021 %>% 
  subset(tipo_atricion == 1) %>% 
  select(starts_with('c46_'), segmento_disenno, estrato_disenno, ponderador02) %>% 
  pivot_longer(cols = starts_with('c46_')) %>% 
  subset(!is.na(value)) %>% 
  prop(value %in% 4:5, by = name) %>% 
  mutate(name=factor(name,
                     levels = c('c46_01','c46_02','c46_03','c46_04'),
                     labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>%
  ggplot(aes(x = name, y = prop,
             label = scales::percent(ifelse(prop>.01, prop, NA), accuracy = .1))) +
  theme_bw() +
  geom_col(fill = viridis::viridis(1)) +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) +
  theme_bw()+
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = 0, end = .85, direction = -1, option = 'viridis')

```

#### Optimismo constitucional según nivel educacional

```{r}

elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & !m01 %in% c(-888, -999)) %>% 
  mutate(educ = factor(car::recode(m01, recodes = "1:3 = 1; 4:5 = 2; 6:7 = 3; 8:10 = 4"),
                     levels = 1:4,
                     labels = c("Basica", "Media", "Tecnica", "Universitaria"))) %>%
  pivot_longer(cols = starts_with('c46_')) %>% 
  filter(!is.na(value)) %>%
  prop(value %in% c(4,5), by = c(educ, name)) %>% 
  mutate(name = factor(name,
                       levels = c('c46_01','c46_02','c46_03','c46_04'),
                       labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>% 
  ggplot(aes(x = name, y = prop, fill = educ,
             label = scales::percent(prop, accuracy = .1))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) + 
  theme_bw() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = 0, end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```

#### Optimismo constitucional según nivel educacional

```{r}

elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & !c15 %in% c(-888, -999)) %>% 
  mutate(pos_id = factor(car::recode(c15, recodes = "0:4 = 1; 5 = 2; 6:10 = 3; 11:12 = 4"),
                       levels = c(1, 2, 3, 4),
                       labels = c('Izquierda', "Centro", "Derecha", "No se identifica"))) %>%
  pivot_longer(cols = starts_with('c46_')) %>% 
  filter(!is.na(value)) %>%
  prop(value %in% c(4,5), by = c(pos_id, name)) %>% 
  mutate(name = factor(name,
                       levels = c('c46_01','c46_02','c46_03','c46_04'),
                       labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>% 
  ggplot(aes(x = name, y = prop, fill = pos_id,
             label = scales::percent(prop, accuracy = .1))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) + 
  theme_bw() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = 0, end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())

```

#### Optimismo constitucional según interés en al politica

```{r}

elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & !c13 %in% c(-888, -999) & ola == 5) %>% 
  mutate(interes = factor(car::recode(c13, recodes = "1:2 = 1; 3 = 2; 4:5 = 3"),
                          levels = 1:3,
                          labels = c('Nada o poco\ninterés', 
                                     'Algo de interés', 
                                     'Bastante o\nmuy interesado'))) %>%
  pivot_longer(cols = starts_with('c46_')) %>% 
  filter(!is.na(value)) %>%
  prop(value %in% 4:5, by = c(name, interes)) %>% 
  sjlabelled::as_label(interes) %>% 
  mutate(name = factor(name,
                       levels = c('c46_01','c46_02','c46_03','c46_04'),
                       labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>% 
  ggplot(aes(x = name, y = prop, fill = interes,
             label = scales::percent(prop, accuracy = .1))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) + 
  theme_bw() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .1, end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())


```

#### Optimismo constitucional según justificación de la violencia para el cambio social (Trabajadores bloqueen las calles)

```{r}
elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & !f05_06 %in% c(-888, -999) & ola == 5) %>%
  mutate(f05_06 = factor(car::recode(f05_06, "1 = 1; 2:3 = 2; 4:5 = 3"),
                         levels = 1:3,
                         labels = c('Nunca se justifica',
                                    'Pocas o algunas veces\nse justifica',
                                    'Muchas veces o siempre\nse justifica'))) %>% 
  pivot_longer(cols = starts_with('c46_')) %>% 
  filter(!is.na(value)) %>%
  prop(value %in% 4:5, by = c(name, f05_06)) %>% 
  sjlabelled::as_label(f05_06) %>% 
  mutate(name = factor(name,
                       levels = c('c46_01','c46_02','c46_03','c46_04'),
                       labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>% 
  ggplot(aes(x = name, y = prop, fill = f05_06,
             label = scales::percent(prop, accuracy = .1))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) + 
  theme_bw() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .1, end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())


```


#### Optimismo constitucional según justificación de la violencia para el cambio social (Trabajadores bloqueen las calles)

```{r}

elsoc_long_2016_2021 %>% 
  filter(tipo_atricion == 1 & !f05_07 %in% c(-888, -999) & ola == 5) %>%
  mutate(f05_07 = factor(car::recode(f05_07, "1 = 1; 2:3 = 2; 4:5 = 3"),
                         levels = 1:3,
                         labels = c('Nunca se justifica',
                                    'Pocas o algunas veces\nse justifica',
                                    'Muchas veces o siempre\nse justifica'))) %>% 
  pivot_longer(cols = starts_with('c46_')) %>% 
  filter(!is.na(value)) %>%
  prop(value %in% 4:5, by = c(name, f05_07)) %>% 
  sjlabelled::as_label(f05_07) %>% 
  mutate(name = factor(name,
                       levels = c('c46_01','c46_02','c46_03','c46_04'),
                       labels = c('Reducirá desigualdad\n en salud y educación','Empeorará condiciones\n económicas',
                                'Reducirá la Corrupción','Poco impacto en la\n Calidad de vida'))) %>% 
  ggplot(aes(x = name, y = prop, fill = f05_07,
             label = scales::percent(prop, accuracy = .1))) +
  geom_col(position = 'dodge2') +
  geom_text(position = position_dodge(.9), vjust = -.5, size = 3) + 
  theme_bw() +
  scale_y_continuous(labels = scales::percent,
                     limits = c(0, 1)) +
  ylab(label = NULL) +
  xlab(label = NULL) +
  scale_fill_viridis_d(begin = .1, end = .85, direction = -1, option = 'viridis') + 
  theme(legend.position = 'top',
        legend.title = element_blank())


```

